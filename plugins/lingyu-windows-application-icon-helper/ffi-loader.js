...
const dllCandidates = [
  path.join(__dirname, 'src', 'bin', 'Release', TFM, 'win-x64', 'native', 'LingyuAppIconHelper.dll'),
  path.join(__dirname, 'src', 'bin', 'Release', TFM, 'win-x64', 'LingyuAppIconHelper.dll'),
];
...
if (!dllPath) {
  throw new Error(
    'Unable to find LingyuAppIconHelper.dll. Run "npm run build" first.'
  );
}
