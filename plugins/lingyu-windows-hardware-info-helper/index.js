...
const helperCandidates = [
  path.join(__dirname, 'src', 'bin', 'Release', 'net10.0', 'LingyuHardwareInfoReader.exe'),
  path.join(__dirname, 'src', 'bin', 'Debug', 'net10.0', 'LingyuHardwareInfoReader.exe'),
];
...
function callHelper(args, timeout = 10000) {
  ...
  if (!helperPath) {
    console.error(
      '[lingyu-windows-hardware-info-helper] Helper executable not found. Searched candidates:',
      helperCandidates,
    );
    return null;
  }
  ...
  if (result.error) {
    console.error(
      '[lingyu-windows-hardware-info-helper] Failed to spawn helper process',
  ...
  if (result.status !== 0) {
    console.error(
      '[lingyu-windows-hardware-info-helper] Helper exited with non-zero status',
  ...
  if (!result.stdout) {
    console.error(
      '[lingyu-windows-hardware-info-helper] Helper produced no stdout',
      { helperPath, args, status: result.status, signal: result.signal, stderr: result.stderr },
    );
    return null;
  }
  ...
  try {
    ...
    if (!Array.isArray(parsed)) {
      console.error(
        '[lingyu-windows-hardware-info-helper] Helper returned non-array JSON payload',
    ...
    console.error(
      '[lingyu-windows-hardware-info-helper] Failed to parse JSON from helper stdout',
