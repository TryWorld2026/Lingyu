...
const dllCandidates = [
  path.join(__dirname, 'src', 'bin', 'Release', TFM, 'win-x64', 'native', 'LingyuBluetoothHelper.dll'),
  path.join(__dirname, 'src', 'bin', 'Release', TFM, 'win-x64', 'LingyuBluetoothHelper.dll'),
];
...
if (!dllPath) {
  throw new Error(
    'Unable to find LingyuBluetoothHelper.dll. Run "npm run build" first.'
  );
}
