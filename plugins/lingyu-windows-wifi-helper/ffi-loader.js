...
const dllCandidates = [
  path.join(__dirname, 'src', 'bin', 'Release', TFM, 'win-x64', 'native', 'LingyuWifiHelper.dll'),
  path.join(__dirname, 'src', 'bin', 'Release', TFM, 'win-x64', 'LingyuWifiHelper.dll'),
];
...
if (!dllPath) {
  throw new Error(
    'Unable to find LingyuWifiHelper.dll. Run "npm run build" first.'
  );
}
