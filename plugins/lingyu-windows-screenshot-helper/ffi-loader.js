...
/** Target framework moniker — keep in sync with LingyuScreenshotHelper.csproj */
const TFM = 'net10.0-windows10.0.19041.0';

const dllCandidates = [
  path.join(__dirname, 'src', 'bin', 'Release', TFM, 'win-x64', 'native', 'LingyuScreenshotHelper.dll'),
  path.join(__dirname, 'src', 'bin', 'Release', TFM, 'win-x64', 'LingyuScreenshotHelper.dll'),
];
...
if (!dllPath) {
  throw new Error('Unable to find LingyuScreenshotHelper.dll. Run "npm run build" first.');
}
