...
const dllCandidates = [
  path.join(__dirname, 'src', 'bin', 'Release', TFM, 'win-x64', 'native', 'LingyuPowerHelper.dll'),
  path.join(__dirname, 'src', 'bin', 'Release', TFM, 'win-x64', 'LingyuPowerHelper.dll'),
];
...
if (!dllPath) {
  throw new Error(
    'Unable to find LingyuPowerHelper.dll. Run "npm run build" first.'
  );
}
