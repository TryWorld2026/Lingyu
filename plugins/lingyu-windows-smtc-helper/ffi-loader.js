...
const dllCandidates = [
  path.join(__dirname, 'src', 'bin', 'Release', TFM, 'win-x64', 'native', 'LingyuSmtcHelper.dll'),
  path.join(__dirname, 'src', 'bin', 'Release', TFM, 'win-x64', 'LingyuSmtcHelper.dll'),
];
...
if (!dllPath) {
  throw new Error(
    'Unable to find LingyuSmtcHelper.dll. Run "npm run build" first.'
  );
}
