...
const api = {
  ...
  /**
   * 将文件路径以 CF_HDROP 格式写入剪贴板（资源管理器中 Ctrl+V 可复制）
   */
  clipboardCopyFiles: (paths: string[]): Promise<boolean> => {
    return ipcRenderer.invoke('clipboard:copy-files', paths);
  },
  /**
   * 读取剪贴板中的文件路径列表
   */
  ...
  toastGetAccessStatus: (): Promise<string> => {
    return ipcRenderer.invoke('toast:get-access-status');
  },
  /**
   * 请求系统通知访问权限
   */
  toastRequestAccess: (): Promise<string> => {
    return ipcRenderer.invoke('toast:request-access');
  },
  /**
   * 启动系统通知接管监听
   */
  toastStart: (): Promise<boolean> => {
    return ipcRenderer.invoke('toast:start');
  },
  /**
   * 停止系统通知接管监听
   */
  toastStop: (): Promise<boolean> => {
    return ipcRenderer.invoke('toast:stop');
  },
  /**
   * 监听系统 Toast 通知事件
   */
  onSystemToast: (callback: (data: { appName: string; title: string; body: string; createdAt: number }) => void): (() => void) => {
    const handler = (_event: Electron.IpcRendererEvent, data: { appName: string; title: string; body: string; createdAt: number }): void => {
      callback(data);
    };
    ipcRenderer.on('system-toast:received', handler);
    return () => {
      ipcRenderer.removeListener('system-toast:received', handler);
    };
  },
  /**
   * 监听系统音量变化事件（音量 HUD）
  ...
  onSystemVolumeChanged: (callback: (data: { volume: number; muted: boolean }) => void): (() => void) => {
    const handler = (_event: Electron.IpcRendererEvent, data: { volume: number; muted: boolean }): void => {
      callback(data);
    };
    ipcRenderer.on('system-volume-changed', handler);
    return () => {
      ipcRenderer.removeListener('system-volume-changed', handler);
    };
  },
  /**
   * 获取电池电量信息
  ...
  systemBatteryGet: (): Promise<{ percent: number; isCharging: boolean; hasBattery: boolean } | null> => {
    return ipcRenderer.invoke('system:battery:get');
  },
  /**
   * 获取剪贴板 URL 监听开关
   */
