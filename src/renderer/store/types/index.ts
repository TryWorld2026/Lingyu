...
/** 灵动岛 UI 状态枚举 */
export type IslandState = 'idle' | 'hover' | 'expanded' | 'notification' | 'maxExpand' | 'lyrics' | 'lyricsTranslation' | 'guide' | 'login' | 'register' | 'resetPassword' | 'setPassword' | 'bindOAuth' | 'bindEmail' | 'payment';
...
/** NowPlaying 原始数据结构（来自 @lingyu/windows-smtc-helper SmtcMonitor） */
...
export interface NotificationData {
  ...
  type?: 'default' | 'source-switch' | 'update-available' | 'update-downloading' | 'update-ready' | 'weather-alert-startup' | 'clipboard-url' | 'restart-required' | 'volume-hud';
...
export interface ShelfSlice {
  ...
  shelfLoaded: boolean;
  shelfDragActive: boolean;
  loadShelfItems: () => Promise<void>;
  persistShelfItems: () => Promise<void>;
  addShelfItems: (paths: string[]) => void;
  removeShelfItem: (path: string) => void;
  clearShelfItems: () => void;
  setShelfDragActive: (active: boolean) => void;
}

/** 完整 Store 类型 */
export type IIslandStore = IslandSlice & WeatherSlice & TimerSlice & MediaSlice & PomodoroSlice & ShelfSlice;