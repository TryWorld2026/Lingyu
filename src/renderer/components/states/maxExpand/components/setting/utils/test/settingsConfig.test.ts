...
describe('normalizeMaxExpandNavLayoutConfig', () => {
  ...
  it('preserves ordering from input', () => {
    const input = [
      ...
      { id: 'clipboardHistory', visible: true },
      { id: 'album', visible: true },
    ];
    const result = mod.normalizeMaxExpandNavLayoutConfig(input);
    expect(result.map((i) => i.id).slice(0, 4)).toEqual(['countdown', 'todo', 'clipboardHistory', 'album']);
  });

  it('allows previously fixed tabs to be hidden', () => {
    const input = [
      { id: 'clipboardHistory', visible: false },
      { id: 'countdown', visible: false },
      { id: 'todo', visible: true },
    ];
    const result = mod.normalizeMaxExpandNavLayoutConfig(input);
    expect(result.find((i) => i.id === 'clipboardHistory')?.visible).toBe(false);
    expect(result.find((i) => i.id === 'countdown')?.visible).toBe(false);
  });
  ...
  it('defaults missing tabs to visible=true', () => {
    const input = [{ id: 'todo', visible: false }, { id: 'clipboardHistory', visible: true }];
    const result = mod.normalizeMaxExpandNavLayoutConfig(input);
    expect(result.find((i) => i.id === 'todo')?.visible).toBe(false);
