...
export type SettingsOpenTabIntent = 'update' | 'about-feedback' | 'user-orders' | 'user-info' | 'ai' | 'performance-monitor' | 'expand-layout';
...
function toMediaUrl(path: string): string {
  ...
  return `lingyu-media://local/${encodeURIComponent(normalized)}`;
}
