...
:::tip
Stores OAuth provider bindings (GitHub, Microsoft, WeChat, Gitee, KOOK) linked to user accounts. Each row represents one third-party account linked to one Lingyu user. A user can have multiple OAuth bindings (one per provider). For the OAuth login flow, see [State Machine — login](../frontend-arch/states.md#github-oauth-login-flow).
:::
...
| `outer_order_no` | VARCHAR(100) | NO | — | Unique order number generated by Lingyu for idempotent request tracking |
...
:::info
Catalog of commonly used software links displayed in the Lingyu toolbox feature. Managed by admins, sorted by `sort_order` for display ordering.
:::
